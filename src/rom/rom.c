...
rom_file_to_buf(const char *rom_path)
{
        ...
        if (!f_ptr)
                log_error("[rom.c: rom_load] fopen failed: %s\n", strerror(errno));
        ...
        if (f_size == -1)
                log_error("[rom.c: rom_load] file size error: %d\n", f_size);
